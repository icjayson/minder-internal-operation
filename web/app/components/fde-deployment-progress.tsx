...
export function FdeDeploymentProgress({ factoryId }: { factoryId: string }) {
  ...
  return (
    <section className="rounded-lg border border-border bg-muted/45 p-3.5">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="text-[9.5px] font-semibold uppercase tracking-[0.12em] text-primary">FDE KIT deployment</div>
          <h3 className="mt-1 truncate text-[13px] font-medium text-foreground">{deployment.name}</h3>
        </div>
        <a
          href={`${process.env.NEXT_PUBLIC_FDE_KIT_URL ?? "https://fde-kit-web.vercel.app"}/deployments/${deployment.id}`}
          target="_blank"
          rel="noopener noreferrer"
          className="shrink-0 rounded-full border border-border-strong px-2.5 py-1 text-[10px] tabular-nums uppercase tracking-wide text-muted-foreground hover:border-primary hover:text-primary transition-colors"
        >
          More details
        </a>
      </div>
      <div className="mt-3 flex items-center gap-2">
        <div className="h-1.5 flex-1 overflow-hidden rounded-full bg-border"><div className="h-full rounded-full bg-primary transition-all" style={{ width: `${percent}%` }} /></div>
        <span className="text-[11px] tabular-nums text-foreground/80">{done}/{tasks.length}</span>
      </div>
