...
export function FactoryTable({
  ...
  const lastActivityColumn: Column<Factory> = {
      ...
      render: (f) => <span className="text-foreground/80 tabular-nums text-[11px] whitespace-nowrap">{formatDate(f.last_activity_at)}</span>,
    };
  const actionsColumn: Column<Factory> = {
      ...
      minWidth: 44,
      render: (f) => (
        <Button variant="ghost" size="icon-sm" onClick={(e) => { e.stopPropagation(); if (confirm(`Delete ${f.name}? This removes its contacts too.`)) onDelete(f.id); }} className="opacity-0 group-hover:opacity-100 w-7 h-7 hover:text-[color:var(--color-danger)] transition-all duration-150" aria-label={`Delete ${f.name}`}>
  ...
  const columns: Column<Factory>[] = customerMode ? [
    accountColumn,
    {
      ...
      render: (f) => <span className="block truncate text-[11.5px] text-foreground/80" title={customerLocation(f)}>{customerLocation(f)}</span>,
    },
    stageColumn,
    {
      key: "contacts",
      ...
      render: (f) => {
        ...
        return <span className="line-clamp-2 text-[11.5px] text-foreground/80" title={names}>{names}</span>;
      },
    },
    lastActivityColumn,
    {
    ...
    dueColumn,
    {
      ...
      render: (f) => <span className="text-foreground/80 tabular-nums">{contactCountFor(f.id)}</span>,
    },
    lastActivityColumn,
    actionsColumn,
  ];
  ...
  return (
    <div title={progress.deploymentName}>
      <div className="flex items-center gap-2">
        ...
        <span className="shrink-0 text-[10.5px] tabular-nums text-foreground/80">{progress.done}/{progress.total}</span>
      </div>
      <div className="mt-1 flex items-center gap-2 truncate text-[9.5px] text-muted-foreground">
        <a
          ...
          className="rounded-full border border-border-strong px-1.5 py-0.5 tabular-nums uppercase hover:border-primary hover:text-primary transition-colors"
        >
          Details
        </a>
