      ...
      <div className="px-4 py-5 sm:px-6 lg:px-8">
        <section className="mb-7" aria-labelledby="today-heading">
          <div className="mb-3 flex items-center justify-between gap-3">
            <div><h2 id="today-heading" className="text-[15px] font-semibold text-ink">Today</h2><p className="text-[11.5px] text-muted-foreground">Start with the accounts that need a decision or follow-up.</p></div>
          ...
          {priorities.length ? (
            ...
            <div className="rounded-card border border-dashed border-line bg-surface/60 px-5 py-7 text-center"><div className="text-[13px] font-medium text-ink">You’re caught up</div><p className="mt-1 text-[11px] text-muted-foreground">No account actions are due today.</p></div>
          )}
        </section>

        <div className="flex items-center justify-between mb-4">
          <div><h2 className="text-[15px] font-semibold text-ink">Explore by vertical</h2><p className="text-[11.5px] text-muted-foreground">Compare portfolio shape and relationship maturity.</p></div>
          ...
          {perVertical.map(({ v, count, grades, ladder }) => (
            <Link key={v.id} href={`/factories?vertical=${v.id}`}
              ...
              <div className="mt-4 pt-3 border-t border-line-soft">
                <div className="text-[9px] mono uppercase tracking-[0.12em] text-muted-foreground mb-1.5">Relationship ladder</div>
                <div className="grid grid-cols-8 gap-1" title="Factories at ladder levels 0–7">
                  {ladder.map((n, level) => (
                    <div key={level} className="text-center">
                      <div className={`h-1.5 rounded-full ${n ? "bg-primary" : "bg-surface-3"}`} />
                      <span className="text-[9px] mono text-muted-foreground">L{level}·{n}</span>
                    </div>
                  ))}
                </div>
              </div>
